...
public class ClientServant implements Util.DownlinkOwner {
    ...
    public synchronized void setDownlink(Downlink paramDownlink) {
     ...
     */
    public synchronized  void downlinkError() {
    ...
    /** Setzt user, benutzt user.setClientServant() - setUser(null) bewirkt ein stopClientServant()
      */
    public synchronized void setUser(User paramUser) {
     ...
     */
    public  synchronized void startClientServant() {
    ...
    /** Sendet das angegebene CommandObjekt über den Uplink.
      * Benutzt uplink.sendMsg()
      * bei einem Fehler: Fehlerbehandlung FIXME: stopClientServant() ? */
    public synchronized void sendCommand(Command paramCommand) {
        try {
            if(this.uplink!=null){
     ...
     * Falls der User Admin-Rechte hat,
     ...
     * Bei einem Loginfehler wird ein LoginErrorCommand gesendet und setUser(null) aufgerufen -> clientServant beendet sich
     */
    public synchronized void loginAsGuest(String name) {
     ...
     */
    public synchronized void becomeAdminClientServant() {
        System.out.println("becoming AdminClientServant");
     ...
     */
    public  synchronized void sendMsgToChannel(String msg) {
        if (this.user != null) {
     ...
     * um bei den verantwortlichen ClientServants ein sendMsgFromChannel() aufzurufen.
     */
    public  synchronized void sendMsgToUser(String userName, String msg) {
        if (this.user != null) {
            if (this.user.getCurrentChannel() != null) {
                Enumeration enum = this.user.getCurrentChannel().getCurrentUserEnum();
                User tmpUser;
     ...
     */
    public  synchronized void sendCurrentChannelData() {
        this.sendCommand(
     ...
     */
    public  synchronized void sendCurrentUserData() {
        this.sendCommand(
     ...
     */
    public  synchronized void setServer(Server paramServer) {
