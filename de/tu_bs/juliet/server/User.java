...
/**
 * Ein Benutzerdatensatz. Enthält alle Informationen über einen Benutzer wie
...
class User {
  ...
  /**
   * Die für den Benutzer verantowrtliche UserAdministration.
   */
   ...
   * der eingeloggt ist und Admin-Rechte bekommt muß sich mittels der
   * AdminClient-Applikation neu einloggen, um diese nutzen zu können.
   */
   ...
   public synchronized void setIsLoggedIn(boolean paramLoggedIn) {
    ...
    if (this.isLoggedIn() != paramLoggedIn) {
      ...
      if (paramLoggedIn) {
        ...
        // Gast bzw. Userzähler in userAdministration erhöhen
        if (this.isGuest()) {
   ...
   * angegebenen Namen zurück. Falls der Channel nicht existiert wird null
   * zurückgegeben.
   */
  ...
  public void removeFromAllowedChannelList(Channel paramChannel) {
    ...
    if (paramChannel != null) {
      ...
      // removeElement() gibt true zurück, falls das Element wirklich entfernt
  ...
  public void setCurrentChannel(Channel paramChannel) {
    ...
    if (this.currentChannel != paramChannel) {
      if (this.currentChannel != null) {
        ...
        /* Assoziation wird temporär "einseitig" gesetzt, damit die Nachricht
         * "verläßt den Channel" zwar an den Channel gesendet wird, vom
        ...
        if (tmpClientServant != null) {
          tmpClientServant.sendMsgToChannel("verläßt den Channel");
        }
