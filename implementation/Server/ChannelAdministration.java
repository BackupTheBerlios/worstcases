...
class ChannelAdministration {
    ...
    public synchronized void setChannelList(Enumeration channelEnum) {
        ...
        if (enum == null) { //Wenn channelEnum leer übergeben wurde,
            enum = (
                new Vector()).elements(); //wird eine neue Liste angelegt.
        }
    ...
    /** Liefert eine Aufzählung der Name der existierenden Channel. Benutzt getChannelEnum(). */
     ...
     */
    public synchronized void editChannel(String oldName, String newName, boolean paramAllowedForGuest,
        Enumeration allowedUser) {
                ...
                //wenn ein Channel mit oldName existiert
                if (tmpChannel != null && tmpChannel.getName().compareTo(this.FOYERNAME)!=0) {
                    //wenn der Name verändert wird, darf ein Channel mit newName nicht existieren
                    if (tmpChannel.getName().compareTo(newName) == 0 | this.getFromChannelListByName(newName) == null) {
